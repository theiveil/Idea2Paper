#!/usr/bin/env bash
# Embedding profile example: DashScope compatible-mode endpoint
# Usage: source profiles/profile_example_dashscope_compatible.sh

if [ "${BASH_SOURCE[0]}" = "$0" ]; then
  echo "Please source this file instead of executing it:"
  echo "  source profiles/profile_example_dashscope_compatible.sh"
  exit 1
fi

export EMBEDDING_PROVIDER="dashscope_compatible"
